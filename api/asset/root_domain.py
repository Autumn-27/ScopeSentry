...
async def asset_data(request_data: dict, db=Depends(get_mongo_db), _: dict = Depends(verify_token)):
    ...
    if len(query) == 0:
        total_count = await db['RootDomain'].count_documents({"_id": {"$exists": True}})
    else:
        total_count = await db['RootDomain'].count_documents(query)
    cursor: AsyncIOMotorCursor = ((db['RootDomain'].find(query, {"_id": 0,
                                                                "id": {"$toString": "$_id"},
                                                                "domain": 1,
                                                                "icp": 1,
                                                                "company": 1,
